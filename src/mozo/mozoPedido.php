...
$tokenResponse = checkToken(); // Keep token check response in a separate variable

$employee_id = $tokenResponse['idEmpleado']; 
$role = $tokenResponse['rol'];
...
try {
    ...
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('iissssdi', $employee_id, $idMesa, $idSucursal, $notas, $horaRecogida, $tipoPedido, $total, $numeroPedidoSucursal);
